    ...
    implements Mage_Shipping_Model_Carrier_Interface
{
    ...
    public function collectRates(Mage_Shipping_Model_Rate_Request $request)
    {
        ...
        /*
        FIX
        @see http://www.magentocommerce.com/boards/viewthread/231771/
        */
        $temp_request = $request;
        $my_rate = 0;
        foreach($temp_request->getAllItems() as $item)
        {
            $temp_request->setPackageWeight($item->getRowWeight());
            $temp_rate = $this->getRate($temp_request);

            // Check the category.
            // For Business Cards we will have a fixed price of $9 regardless the region.
            $category = $item->getProduct()->getCategoryIds();
            if($category[0] == 8) {
                $temp_rate['price'] = self::SHIPPING_BUSINESCARDS;
            }

            $my_rate += ($temp_rate['price']);
        }
